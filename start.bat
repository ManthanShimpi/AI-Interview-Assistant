...
start "AI Backend Service (FastAPI)" cmd /k "cd backend && if not exist venv (python -m venv venv) && call venv\Scripts\activate && echo Installing heavy AI dependencies. This CAN take 10+ minutes depending on your internet speed... && pip install -r requirements.txt && echo Downloading local NLP model... && python -m spacy download en_core_web_sm && echo Starting Server... && uvicorn main:app --reload --host 0.0.0.0 --port 8000"

start "AI Frontend UI (React/Vite)" cmd /k "cd frontend && echo Installing Frontend Dependencies... && call npm install && echo Starting UI Server... && npm run dev"
